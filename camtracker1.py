__author__ = "esdalmaijer + Siddhant Shrivastava + Karan Katyal"
...
print(filename)
# filename = "http://htdp.org/2003-09-26/Book/curriculum-Z-H-49.html"
import autopy
import os.path
...
class Setup:
	...
	def show_welcome(self, loading=False):
		...
		# welcome text
		welcometext = \
		"""Welcome to BITS Apogee 2015!
		
		Author: Team GUEye
		Project: Natural User Interface for Computers
	...
	def handle_input(self, inptype, inp, stage, stagevars):
		...
		# stage 3
			...
			#	autopy.mouse.move(x*3,y*2)
			if x>400:
				#self.m+=20
				#driver.execute_script("window.scrollTo(0,2000)")
				#while m!= 1000:
        				#time.sleep(1)
        			#command = "window.scrollTo(0," + str(self.m) + ")"
				elem = driver.find_element_by_class_name("slides")
			...
			if x<200:
				...
				#while m!= 1000:
        				#time.sleep(1)
        			#command = "window.scrollTo(0," + str(self.m) + ")"
				elem1 = driver.find_element_by_class_name("slides")
				elem1.send_keys("",Keys.ARROW_LEFT)
			'''
        		if y<100:
				self.m-=20
				#driver.execute_script("window.scrollTo(0,2000)")
			...
			'''
				if y<100:
					...
					while n!= 1000:
        					#time.sleep(1)
        					command = "window.scrollTo(0," + str(n) +")"
	        				driver.execute_script(command)
        					n -= 10
